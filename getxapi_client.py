"""Optional GetXAPI backend client for the twitter MCP server."""
...
class GetXAPIError(RuntimeError):
    """Raised when the optional GetXAPI backend cannot complete a request."""
...
class GetXAPIClient:
    ...
    @classmethod
    def from_env(cls, env: Mapping[str, str] | None = None) -> "GetXAPIClient":
        source = env if env is not None else os.environ
        api_key = source.get("GETXAPI_API_KEY") or ""
        base_url = source.get("GETXAPI_BASE_URL") or DEFAULT_BASE_URL
        account = source.get("GETXAPI_ACCOUNT") or ""
        actions_enabled = source.get("GETXAPI_ENABLE_ACTIONS", "").lower() in {
            "1",
            "true",
            "yes",
        }
    ...
    def get_tweets(self, username: str, count: int) -> list[dict[str, Any]]:
        payload = self._request_json(
            ...
            query={"q": f"from:{username.lstrip('@')}", "queryType": "Latest", "limit": count},
        )
        return [
    ...
    def get_profile(self, username: str) -> dict[str, Any]:
        payload = self._request_json(
            "GET",
            "/twitter/user/info",
            query={"userName": username.lstrip("@")},
        )
        profile = self._extract_object(payload, "profile", "user", "data")
    ...
    def post_tweet(self, text: str, reply_to_tweet_id: str = "") -> dict[str, Any]:
        if not self.can_create_tweets():
            raise GetXAPIError(
                "GetXAPI posting requires GETXAPI_API_KEY, GETXAPI_ACCOUNT, and GETXAPI_ENABLE_ACTIONS=true."
            )
        ...
        payload = self._request_json("POST", "/twitter/tweet/create", body=body)
    ...
    def _request_json(
        ...
        if not self.api_key:
            raise GetXAPIError("GetXAPI backend is not configured.")
        ...
        try:
            ...
            raise GetXAPIError(
                f"GetXAPI request failed with HTTP {exc.code}: {detail}"
            ) from exc
        except URLError as exc:
            raise GetXAPIError(f"GetXAPI request failed: {exc.reason}") from exc
